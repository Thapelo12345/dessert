...
function createCartItems(dessert) {
  ...
  deleteBtn.click(function(){
    ...
    $('#counter').text( Number($('#counter').text()) - Number(currentList.find('.qauntity').text().split('x')[0]))
    //done setting counter

    let priceSetup = Number($('#purchase').find('#amount').text().split('$')[1]) - Number(currentList.find('.item-total').text().split('$')[1])
    let finalPriceSet = priceSetup.toString().split('.').length === 1 ? priceSetup + '.00' : priceSetup + '0'
    $('#purchase').find('#amount').text('$' + finalPriceSet)

    currentList.empty().remove()
...
function incrementDecrement(crrValue, crrContainer) {
  ...
  let addPrice = Number(listItem.find(".item-total").text().split("$")[1]);
